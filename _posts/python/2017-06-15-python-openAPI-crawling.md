...
## 2. 네이버의 Open API를 활용한 그림 다운로드 받는 크롤러 만들기
...
`urllib.parse`는 요청할 url에 들어가는 검색어를 인코딩 시켜주기 위한 모듈입니다.  
`quote`함수는 공백을 `%20`으로, `quote_plus`함수는 공백을 `+`으로 치환합니다.  
`urllib.request`는 주어진 주소에 HTML 코드를 요청을 할 수 있도록 해주는 모듈입니다.  
`re`모듈은 Python에서 정규식을 사용하기 위한 모듈입니다.  

`fetch_contents_from_url`함수는 이름 그대로 url로부터 콘텐츠를 가져오는 역할을 합니다. `bind_url`함수를 실행시켜 하나로 묶인 url을 `url`변수에 저장하고 `urlopen`함수를 이용해서 요청을 합니다. 이렇게 하면 `r`에 `request`객체가 들어가게 되고 `request`객체에서 `read`함수를 사용하면 HTML(웹 데이터)를 읽어 들입니다. 그렇게 읽어 들인 HTML(웹 데이터)코드를 `html`변수에 저장하고 변환시켜 줍니다.  

`extract_text_in_tags`함수는 HTML에서 꺼낸 태그들을 리스트로 넘겨주면 정규식을 사용해서 텍스트만 남겨주는 역할을 합니다. 예를 들어서

```html
<title>Naver Open API</title>
```
위와 같은 태그가 있을때 정규식을 사용하면 "<title>"을 제외하고 '<'기호 전의 문자들만 모두 걸러질 것입니다. 그러면 이 걸러진 문자열을 txt라는 리스트에 추가시켜두고 for문이 실행되서 모든 태그의 텍스트를 전부 걸러내고 txt 리스트를 반환합니다.  

`get_contents_from_html`함수는 우선 `fetch_contents_from_url`함수로부터 html을 반환 받아 html변수에 저장합니다. 그리고 title태그와 link태그를 걸러내기 위해서 정규식을 하용합니다. `decode("utf-8")`함수를 사용하는 이유는 html을 `read()`함수로 읽어오면 인코딩이 한글로 보는게 불가능하기 때문에 `decode`함수를 사용해서 한글을 볼 수 있게 하는 것입니다.  

이렇게 title과 link태그들을 찾아서 리스트로 저장하고 위에서 생성했던 `extract_text_in_tags`함수를 사요앻서 추출해낸 title태그와 link태그의 텍스트를 가져옵니다.  
그리고 `open`함수를 통해서 현재 작업경로에 `image.html`이라는 파일을 쓰기 모드로 생성하고 html문서를 만들기 위한 html태그와 가져온 콘텐츠들을 표시해줄 body태그를 `write`함수를 통해서 넣어줍니다.  


**하지만**

네이버 API 사용법이 바뀌었기 때문에 저대로 하면 에러가 발생합니다.
...
get_contents_from_html()
```
이전에 사용했던 것처럼 헤더를 추가해 요청해보았지만, 여전히 에러가 발생했습니다.

어험... 무엇을 하던지 꼭 벽에 막히지만 그것을 푸는게 묘미.  

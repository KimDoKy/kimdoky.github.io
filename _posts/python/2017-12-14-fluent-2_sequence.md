...
### 2.9.4 덱 및 기타 큐
`append(), pop()` 메서드를 사용해서 리스트를 스택이나 큐(`append()`와 `pop()`을 사용하면 FIFO 방식으로 작동합니다.)로 사용할 수 있습니다. 그러나 리스트 왼쪽(0번 인덱스)에 삽입하거나 삭제하는 연산은 전체 리스트를 이동시켜야하므로 처리 부담이 큽니다.  

덱(`collections.deque`) 클래스는 큐의 양쪽 어디에서든 빠르게 삽입 및 삭제할 수 있도록 설계된 안전한 스레드(thread-safe) 양방향 큐입니다. '최근에 본 항목'이나 이와 비슷한 것들의 목록을 유지할 때도 사용할 수 있습니다. 덱은 최대 길이를 설정해서 제한된 항목만 유지할 수도 있으므로 덱이 꽉 찬 후에는 새로운 항목을 추가할 때 반대쪽 항목을 버립니다.

```python
# 덱 이용하기
>>> from collections import deque
# 선택적 인수 maxlen은 덱 객체가 수용할 수 있는 최대 항목 수를 설정한다. 덱 객체를 생성할 때 읽기 전용 속성인 maxlen을 설정한다.
...
# 오른쪽 항목 3개를 추가하면 왼쪽 끝에 있는 -1, 1, 2가 밀려나간다.
>>> dq.extend([11, 22, 33])
>>> dq
...
---|---|---|---
`s.__add__(s2)` |√| |s + s2 - 연결한다.
`s.__iadd__(s2)` |√|√|s += s2 - 연결한 후 s에 저장한다.
...
`s.extend(i)` |√|√|반복 가능한 i에 있는 요소를 오른쪽에 추가한다.
`s.extendleft(i)` | |√|반복 가능한 i에 있는 요소를 왼쪽에 추가한다.
`s.__getitem__(p)` |√|√|s[p] - p 위치의 항목을 가져온다.
`s.index(e)` |√| |처음 e가 나타난 위치를 반환한다.
`s.insert(p, e)` |√| |p 위치의 항목 옆에 e 요소를 추가한다.
`s.__iter__()` |√|√|반복자를 반환한다.
`s.__len__()` |√|√|len(s) - 항목 수를 반환한다.
`s.__mul__(n)` |√| |s * n - 반복해서 연결한다.
`s.__imul__(n)` |√| |s \*= n - 반복해서 연결한 후 s에 저장한다.
`s.__rmul__(n)` |√| |n * s - 역순 반복 연결
`s.pop()` |√|√|마지막 항목을 삭제하고 반환한다.
`s.popleft()` | |√|첫 번째 항목을 삭제하고 반환한다.
`s.remove(e)` |√|√|e와 같은 값을 가진 첫 번째 항목을 삭제한다.
`s.reverse()` |√|√|항목을 역순으로 나열해서 다시 s에 저장한다.
`s.__reversed__()` |√|√|뒤에서부터 앞으로 나열하는 반복자를 반환한다.
`s.rotate(n)` | |√|한쪽에 있는 n개의 항목을 반대쪽으로 이동시킨다.
`s.__setitem__(p, e)` |√|√|s[p] = e - p 위치에 e 값을 저장하고, 기존 값을 덮어쓴다.
`s.sort([key], [reverse])` |√| | 선택적인 키워드 인수 key와 reverse에 따라 항목을 정렬하고 s에 저장한다.

덱은 리스트 메서드 대부분을 구현할 뿐만 아니라 `popleft(), rotate()`처럼 고유한 메서드를 추가로 가지고 있습니다. 하지만 덱은 중간 항목을 삭제하는 연산을 빠르지 않습니다. 덱이 양쪽 끝에 추가나 제거하는 연산에 최적화되어 있습니다.  

`append(), popleft()`메서드느 원자성을 갖고 있으므로 멀티스레드 앱에서 락을 사용하지 않고도 덱을 이용해서 간단히 FIFO 큐를 구현할 수 있습니다.  

다른 파이썬 표준 라이브러리 패키지에서는 덱 외에도 다른 큐들을 구현하고 있습니다.

#### queue
`queue` 모듈에서는 동기화된(스레드 안전한) Queue, LifoQueue, PriorityQueue 클래스를 제공합니다. 이 클래스들은 스레드 간에 안전하게 통신하기 위해 사용됩니다. 세 클래스 모두 0보다 큰 maxsize 인수를 생성자에 전달해서 바인딩할 수 있습니다. 그렇지만 덱과 달리 공간이 꽉 찼을 대 항목을 버리진 않습니다. 대신 새로운 항목의 추가를 블로킹하고 다른 스레드에서 큐 안의 항목을 제거해서 공간을 확보해줄 때까지 기다립니다. 따라서 활성화된 스레드 수를 조절하기 좋습니다.

#### multiprocessing
멀티프로세싱 모듈은 queue.Queue와 비슷하지만 프로세스 간 통신을 지원하기 위해 설계된 고유 Queue 클래스를 구현합니다. 태스크 관리에 특화된 multiprocessing.JoinableQueue 클래스도 제공합니다.

#### asyncio
asyncio 모듈은 queue 및 multiprocessing 모듈에 포함된 클래스로부터 영감을 얻은 Queue, LifoQueue, PriorityQueue, JoinableQueue 클래스를 제공하지만, 비동기 프로그래밍 환경에서 작업을 관리하는데 주안점을 주고 있습니다.

#### heapq
앞의 3개의 모듈과는 다르게 heapq는 queue 클래스를 구현하지 않지만, 가변 시퀀스를 힙 큐나 우선순위 큐로 사용할 수 있게 해주는 heappush()와 heappop() 등의 함수를 제공합니다.

## 2.10 요약
표준 라이브러리에서 제공하는 시퀀스형을 제대로 파악하고 있어야 간결하고, 효율적이고, 파이썬다운 코드를 작성할 수 있습니다.  

파이썬 시퀀스는 가변형과 불변형으로 구분하기도 하지만,  균일 시퀀스와 컨테이너 시퀀스로 분휴하는 것도 도움이 됩니다. 균일 시퀀스는 작고, 빠르고, 사용하기 쉽지만 숫자, 문자, 바이트처럼 원자적인 데이터만 저장할 수 있습니다. 컨테이너 시퀀스는 융통성이 있지만, 가변 객체를 저장할 때는 예상치 못한 일이 발생할 수도 있습니다. 따라서 내포된 데이터 구조체와 함께 컨테이너 시퀀스를 사용할 때는 주의해야 합니다.  

지능형 리스트와 제네레이터 표현식은 시퀀스를 생성하고 초기화하는 강력한 표기법입니다.  

파이썬이 제공하는 튜플은 익명 필드를 가진 레코드 및 불변 리스트로 사용할 수 있습니다. 튜플은 레코드로 사용할 때는 튜플 언패킹이 필드에 접근하는 가장 안전하고 가독성이 좋은 방법입니다. 새로 추가된 \* 구문을 이용하면 일부 필드를 무시하거나 선택적 필드를 처리하기 상당히 좋습니다. 명명된 튜플은 튜플과 마찬가지로 객체당 오버헤드가 적으며, 이름을 이용해서 간단히 필드에 접근할 수 있고, `_asdict()` 메서드를 사용하면 레코드를 `OrderedDict` 객체로 익스포트할 수도 있습니다.  

시퀀스 슬라이싱은 파이썬에서 즐겨 사용하는 구문 기능으로서 생각보다 훨씬 강력합니다. NumPy에서 사용되는 다차원 슬라이싱과 생략기호(...) 표기도 사용자 정의 시퀀스에서 지원할 수 있습니다. 슬라이스에 할당하는 구문은 가변 시퀀스의 편집을 멋지게 표현합니다.  

seq * n으로 표현되는 반복 연결은 편리하게 사용할 수 있으며, 주의해서 사용하면 가변 항목을 담은 리스트의 리스트를 초기화할 수도 있습니다. +=와 \*= 복합 할당 연산자는 가변/불변 시퀀스 여부에 따라 다르게 작동합니다. 복합 할당 연산자는 대상 시퀀스가 불변인 경우에는 새로운 시퀀스를 생성합니다. 그러나 대상 시퀀스가 가변인 경우에는 대상 시퀀스를 직접 변경합니다. 하지만 시퀀스 구현 방식에 따라 그렇지 않을 수도 있습니다.  

`sort()`메서드와 `sorted()` 내장 함수는 사용하기 쉬우며, 선택적인 key 인수에 정렬 기준을 계산하는 함수를 지정할 수 있으므로 융통성도 뛰어납니다. 한편 key는 `min(), max()` 내장 함수와도 사용할 수 있습니다. 정렬된 시퀀스의 순서를 유지하면서 항목을 추가하려면 `bisect.insert()` 메서드를 사용하고, 정렬된 시퀀스를 효율적으로 검색하려면 `bisect.bisect()` 메서드를 사용합니다.  

파이썬 표준 라이브러리는 리스트와 튜플 외에 `array.array`도 제공합니다. NumPy와 SciPy는 표준 라이브러리에 속하지는 않지만, 대형 데이터셋에 수치 연산을 수행하는 경우에는 이런 라이브러리를 약간만 알아두어도 큰 도움이 됩니다.  

기능이 풍부하고 스레드 안전한 `collections.deque`에 대해 살펴봤고, 리스트와 API를 비교해보고, 표준 라이브러리에서 구현하는 여러 큐 클래스에 대해 간략히 다루었습니다.
